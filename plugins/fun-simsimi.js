...
let handler = async (m, { conn, text, usedPrefix, command }) => {
 ...
 let name = conn.getName(m.sender)
  ...
  //let res = await fetch(global.API('https://api.simsimi.net', '/v2/', { text: encodeURIComponent(m.text), lc: "it" }, ''))
  let res = await fetch(`https://api.simsimi.net/v2/?text=${text}&lc=it`)
